...
if(global.move_action2 and takeoverable != noone)
{
...
if(global.move_up)    yspd = -SMOKESPEED
if(global.move_down)  yspd =  SMOKESPEED
if(!global.move_up and !global.move_down)    yspd = 0
if(global.move_left)  xspd = -SMOKESPEED
if(global.move_right) xspd =  SMOKESPEED
if(!global.move_left and !global.move_right) xspd = 0
...
/*switch state
{
	...
	//{
		...
		if(!just_created and collision_rectangle(x - (SMOKEWIDTH / 2), y + (sign(yspd) * SMOKEWIDTH / 2),
		                       x + (SMOKEWIDTH / 2), y + (sign(yspd) * SMOKEWIDTH / 2) + yspd,
							   oBarrier, false, false))
	    {
			while(!collision_rectangle(x - (SMOKEWIDTH / 2), y + (sign(yspd) * SMOKEWIDTH / 2),
		                               x + (SMOKEWIDTH / 2), y + (sign(yspd) * SMOKEWIDTH / 2) + 1,
							           oBarrier, false, false))
			{
				y += (sign(yspd) * 1)
			}
